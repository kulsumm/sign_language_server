...
for video_folder in os.listdir(dataset_root):
    folder_path = os.path.join(dataset_root, video_folder)
        ...
        while True:
            ...
            for key in [res.pose_landmarks, res.face_landmarks, res.left_hand_landmarks, res.right_hand_landmarks]:
                if key:
                    ...
                    frame_vec.extend([0]*21*3)  # adjust 21 landmarks for hands/pose
            frames.append(frame_vec)
    cap.release()
...
# 3. Compare uploaded video
def find_best_match(upload_features, dataset_features):
    best_vid = None
    best_dist = float('inf')
    for vid, features in dataset_features.items():
        dist, _ = fastdtw(upload_features, features, dist=euclidean)
...
def predict():
    ...
    threshold = 100  # tune empirically
    if dist < threshold:
        predicted_gloss = glosses[best_vid]
        predicted_translation = translations[best_vid]
    else:
